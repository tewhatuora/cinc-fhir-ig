...
Given the large number of ongoing resources(Questionnaire Responses and Observations) that are generated by Patients, it may be required for clients to create further consents beyond the one created at patient onboarding time, as these can grow too large. When a client GETs a consent to update it with further data provisions, if it find that this consent will be greater than 10,000 lines (as a rule of thumb), the client should create a new consent.

<figure>
  <!-- Generated from `input/images-source/rpm-consentRotation.plantuml` -->
  {% include rpm-consentRotation.svg %}
</figure>
<br clear="all">
